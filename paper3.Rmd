...
X_train <- train_set %>% 
  dplyr::select(Scenicness = Scenicness.weighted_bayescore,
                Wildness = Wildness.median, x_coord, y_coord)
 X_test <-  test_set %>% 
   dplyr::select(Scenicness = Scenicness.weighted_bayescore,
                 Wildness = Wildness.median, x_coord, y_coord)
...
```{python plot}
X =  r.landmap.loc[:, ['Scenicness.median_of_bayescore', 'Wildness.median', 'x_coord', 'y_coord']]
# -- play shap
...
```{r}
# -- download the administrative region for Great Britain from GADM
...
# -- spatial join OSGB 1 km grid with Scenic-Or-Not data
test <- st_join(grid_1km, sc) 
library(dplyr)

test1 <- test |>
  st_drop_geometry() |>
  group_by(PLAN_NO) |>
  summarise(ID = paste2(ID, collapse = ', '),
...
test2 <- test1 |>  
  mutate(Photos = strsplit(ID, ',') |> unlist() |> !is.na() |> length())

strsplit(",") %>% unlist() %>% as.numeric() %>% mean()
...
#   summarize_(text=~paste(text, collapse=' '))
  ...
  aggregate(#cbind(# ID,
                  ...
                  # BayeScore,
                  # Entropy,
                  Variance~PLAN_NO, data = ., FUN = paste, collapse = ",") %>%
  merge(grid_1km, ., by = "PLAN_NO", all = T) |>
  as_tibble()  
  
plot(UK['NAME_0']) +
  plot(test[is.na(test$ID),'Mean'])

grid_1km <- st_join(grid_1km, sc, join = st_within, left = TRUE) |>
