...
LANDMAP <- sc %>% 
  ...
  st_as_sf() %>%
  rowwise() %>%
  mutate(Scenicness.mean_of_all = Votes %>% strsplit(",") %>% unlist() %>% as.numeric() %>% mean(),
         Scenicness.median_of_all = Votes %>% strsplit(",") %>% unlist() %>% as.numeric() %>% median(),
         Scenicness.entropy_of_all = Votes %>% strsplit(",") %>% unlist() %>% as.numeric() %>% entropy(),
         Scenicness.number_of_all = Votes %>% strsplit(",") %>% unlist() %>% as.numeric() %>% na.omit() %>% length()) %>%
  ungroup()
...
LANDMAP <- sc %>%
  st_join(LANDMAP, .) %>%
  aggregate(BayeScore~UID, data = ., paste0, collapse = ",") %>%
  merge(LANDMAP, ., by = "UID", all = T) %>%
  as_tibble() %>%
  st_as_sf() %>%
  rowwise() %>%
  mutate(Scenicness.mean_of_bayescore = BayeScore %>% strsplit(",") %>% unlist() %>% as.numeric() %>% mean(),
         Scenicness.median_of_bayescore = BayeScore %>% strsplit(",") %>% unlist() %>% as.numeric() %>% median(),
...
LANDMAP <- st_transform(LANDMAP, crs = st_crs(Wildness)) %>%
  ...
  dplyr::select(UID, CLS_1, CLS_2, CLS_3, CLS_4, ScenicQuality, Consultant,
                Votes, Scenicness.mean_of_all, Scenicness.median_of_all,
                Scenicness.entropy_of_all, Scenicness.number_of_all,
                Scenicness.mean_of_median, Scenicness.median_of_median,
                Scenicness.mean_of_mean, Scenicness.median_of_mean,
                Scenicness.mean_of_bayescore, Scenicness.median_of_bayescore,
                Wildness.mean = mean, Wildness.median = median, Wildness.iqr, 
                Wildness.cv = coefficient_of_variation, Wildness.variance = variance) %>%
  st_transform(crs = 27700)
...
X = landmap_B.loc[:, ['Scenicness.median_of_bayescore', 'Wildness.median', 'x', 'y']]
...
disp = ConfusionMatrixDisplay.from_predictions(model, X_test, y_test, 
...
# plt.gcf().axes[-1].set_box_aspect(200) 
